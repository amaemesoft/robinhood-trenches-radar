...
test('social discovery can build culture but never smart-money score',()=>{
  ...
  assert.equal(r.components.money,null);
...
test('unknown holder and resilience data are null and never receive placeholder credits',()=>{
  const r=Cycle.evaluateCycleToken({actors,safety,execution,token:{marketCap:12e6},now,events:[
    {actorId:'s1',action:'SCOUT',source:'fxtwitter',at:atDays(1)}
  ]});
  assert.equal(r.components.holders,null);
  assert.equal(r.components.resilience,null);
  assert.ok(r.coverage<1);
  assert.ok(r.evidence.scoreCoveragePct<100);
  assert.ok(r.missingData.includes('holder_growth'));
  assert.ok(r.missingData.includes('long_horizon_resilience'));
});

test('strong measured holders/resilience increase evidence-aware score versus the same unknown evidence',()=>{
  const events=[
  ...
  const base={actors,safety,execution:{...execution,exitabilityScore:95},token:{marketCap:35e6},now,events};
  const unknown=Cycle.evaluateCycleToken(base);
  const measured=Cycle.evaluateCycleToken({...base,cycleMetrics:{
  ...
  assert.ok(measured.score>unknown.score);
  assert.ok(measured.coverage>unknown.coverage);
});
...
test('missing holder and resilience history caps generational stages',()=>{
  const r=Cycle.evaluateCycleToken({actors,safety,execution,token:{marketCap:40e6},now,events:[
    ...
    {actorId:'s1',action:'SCOUT',source:'fxtwitter',at:atDays(1)},{actorId:'s2',action:'SCOUT',source:'telegram',at:atDays(1)}
...
test('measured holders plus post-crash resilience can unlock upper-stage gate without tactical semantics',()=>{
  ...
  assert.ok(r.coverage>=0.75);
});

test('unknown safety can never unlock chain-icon gate',()=>{
  const r=Cycle.evaluateCycleToken({actors,safety:{},execution:{...execution,exitabilityScore:95},token:{marketCap:35e6},now,events:[
    {actorId:'s1',action:'SCOUT',source:'fxtwitter',at:atDays(20)},{actorId:'s2',action:'SCOUT',source:'telegram',at:atDays(2)}
  ],cycleMetrics:{
    holders:{status:'MEASURED',score:95,confidence:.9,currentHolders:30000},organic:{status:'MEASURED',score:90},
    resilience:{status:'MEASURED',score:90,confidence:.9},marketStructure:{status:'MEASURED',score:90,confidence:1}
  }});
  assert.equal(r.gates.chainIconGate,false);
});

test('hypervelocity with weak holder absorption gets meteor warning rather than durability credit',()=>{
  const recent=Array.from({length:10},(_,i)=>({actorId:i%2?'s1':'s2',action:'SCOUT',source:i%2?'fxtwitter':'telegram',at:new Date(now-i*20*60e3).toISOString()}));
  ...
  assert.ok(r.analogues.some(a=>['BODEN','BOME','PNUT','MOODENG'].includes(a.key)));
  assert.ok(r.score<=84);
  assert.ok(r.risks.some(x=>x.includes('not been tested')));
});
