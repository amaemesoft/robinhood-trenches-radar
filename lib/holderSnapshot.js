...
function topHolderConcentration(items=[],totalSupplyRaw,options={}){
  ...
  // Basis points first avoids unsafe Number(totalSupplyRaw) conversion for 18-decimal supplies.
  const bps=Number((sum*1000000n)/supply)/100;
  return{
    top10Pct:Number(Math.max(0,Math.min(100,Number(bps.toFixed(4))))),
