...
function buildAnalogues({culture=0,holders=null,resilience=null,attention=0,market=0,organic=null,ageDays=null,resilienceStatus='UNKNOWN',sourceDiversity=0}={}){
  ...
  const h=Number.isFinite(Number(holders))?Number(holders):null;
  const r=Number.isFinite(Number(resilience))?Number(resilience):null;
  const o=Number.isFinite(Number(organic))?Number(organic):null;
  const age=Number.isFinite(Number(ageDays))?Number(ageDays):null;

  if(culture>=78&&h!=null&&h>=68&&attention>=72){
    add('PEPE','MEDIUM','culture, holder adoption and attention are expanding together');
  }
  if(culture>=72&&market>=58&&(o==null||o>=55)&&sourceDiversity>=1){
    add('WIF','MEDIUM','portable social propagation is combining with tradable market depth without obvious holder-price divergence');
  }
  if(age!=null&&age>=30&&h!=null&&h>=62&&r!=null&&r>=68){
    add('POPCAT','HIGH','candidate has persisted long enough for holder retention and recovery to matter');
  }
  if(r!=null&&r>=78&&h!=null&&h>=68&&market>=60){
    add('BONK','HIGH','holder base and market structure persisted through a material drawdown');
  }
  if(age!=null&&age>=45&&culture>=62&&h!=null&&h>=62&&r!=null&&r>=55){
    add('MEW','MEDIUM','slow category expansion is being confirmed by holders and persistence rather than launch velocity');
  }
  if(culture>=72&&sourceDiversity>=2&&o!=null&&o>=65&&attention>=58){
    add('FARTCOIN','MEDIUM','simple social identity is broadening across sources while holders absorb the move');
  }

  // Meteor/hypervelocity analogues are warnings, not bullish endorsements.
  if(age!=null&&age<=14&&attention>=84&&resilienceStatus!=='MEASURED'){
    add('BOME','LOW','launch/attention velocity is extreme but durability has not been tested','hypervelocity can be a false positive and a late-entry trap');
  }
  if(age!=null&&age<=45&&attention>=78&&o!=null&&o<45&&resilienceStatus!=='MEASURED'){
    add('BODEN','LOW','attention is outrunning holder absorption before resilience is known','meteor risk: narrative may decay faster than the holder base forms');
  }
  if(age!=null&&age<=21&&attention>=82&&culture>=62&&sourceDiversity>=2&&resilienceStatus!=='MEASURED'){
    add('PNUT','LOW','real-world-style viral acceleration is plausible from the speed and breadth of attention','verify that culture persists after the external news cycle');
  }
  if(age!=null&&age<=30&&attention>=72&&culture>=68&&sourceDiversity>=2&&resilienceStatus!=='MEASURED'){
    add('MOODENG','LOW','broad viral meme propagation is arriving before durability is measurable','mainstream virality can fade quickly if on-chain retention does not form');
  }
