...
async function resolveTokenIdentity(rpc,address,fetchImpl=globalThis.fetch){
  ...
  if(contractExists===true){
    ...
    const name=rpcName||dex.name||null;
    const symbol=cleanTicker(rpcSymbol)||dex.symbol||null;
    if(name||symbol||dex.verified===true)return{contractExists:true,name,symbol};
