...
function marketStructure(history=[]){
  const rows=(history||[]).map(r=>({
    at:r?.at,ms:Date.parse(r?.at||''),marketCap:Number(r?.marketCap),liquidityUsd:Number(r?.liquidityUsd),
    volume24h:Number(r?.volume24h),buys24h:Number(r?.buys24h),sells24h:Number(r?.sells24h)
...
module.exports={holderPriceDivergence,marketStructure,normalizedHolderRows,clamp};
