...
function buildCycleBoard({db,actors,cycleRuntime,tacticalSignals=[],now=Date.now(),recordScores=false,windowDays=Universe.DEFAULT_WINDOW_DAYS}={}){
  ...
  for(const candidate of universe){
    ...
    const firstSeenAt=earliest([
      ...events.map(e=>e.at),marketHistory[0]?.at,holderHistory[0]?.at,st.observedAt
    ]);
    ...
    const lastEvidenceAt=latest([
      ...events.map(e=>e.at),marketHistory.at(-1)?.at,holderHistory.at(-1)?.at,holdings.observedAt,st.observedAt
    ]);
    ...
    out.push({
      ...
      cyclePotential:result.score,cycleStage:result.stage,cycleVersion:result.version,cycleComponents:result.components,
  ...
  return out.sort((a,b)=>Number(b.cyclePotential||0)-Number(a.cyclePotential||0)||Number(b.cycleDelta24h||0)-Number(a.cycleDelta24h||0)||Number(b.cyclePinned)-Number(a.cyclePinned));
}
