...
function buildCycleUniverse({events=[],tokenState={},holderHistory={},marketHistory={},seeds=CYCLE_SEEDS,now=Date.now(),windowDays=DEFAULT_WINDOW_DAYS}={}){
  ...
  for(const [address,state] of Object.entries(tokenState||{}))if(state?.contractExists!==false)add(address,{symbol:state?.tokenSymbol,name:state?.tokenName,reason:'known-token-state'});
