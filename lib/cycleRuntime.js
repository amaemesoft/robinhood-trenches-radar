...
class CycleRuntime{
  constructor({holderProvider,holdingsProvider,snapshotIntervalMs=60*60*1000,holdingsIntervalMs=2*60*60*1000,maxHolderSnapshots=400,maxScoreSnapshots=400}={}){
    ...
    this.maxScoreSnapshots=Math.max(100,Number(maxScoreSnapshots)||400);
  }
  ...
  recordScore(db,address,score,stage,at=new Date().toISOString()){
    ...
    const ms=Date.parse(at),lastMs=Date.parse(last?.at||'');
    // Full Cycle refresh can be triggered by faster tactical workers. Score history is a
    // slow signal: never rewrite the latest sample more often than every 15 minutes.
    // This prevents a no-op 2 minute heartbeat from dirtying hundreds of histories and
    // forcing a full radar_state JSONB rewrite.
    if(last&&Number.isFinite(ms)&&Number.isFinite(lastMs)&&ms>=lastMs&&ms-lastMs<15*60*1000)return false;
    history.push({at,score:Number(score),stage:stage||null});
