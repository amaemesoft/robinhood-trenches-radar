  ...
  async analyzeTx(txHash,walletMap){
    ...
    for(const wallet of involved){
      ...
      for(const [token,delta] of nonzero){
        ...
        if(QUOTE_SYMBOLS.has(String(p.symbol||'').toUpperCase()))continue;
        ...
        rows.push({actorId:walletMap.get(wallet).id,wallet,action,tokenAddress:token,symbol:p.symbol,amountRaw:delta.toString(),amount:formatUnits(delta,p.decimals),txHash,blockNumber,blockHash,at:timestamp,source:'blockscout-pro',signalRole:action==='BUY'?'execution':null,classificationEvidence:{otherTokenOut:otherOut,otherTokenIn:otherIn,nativeSpent}});
      }
    }
    return rows;
  }
