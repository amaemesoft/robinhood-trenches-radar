...
function measuredMetric(metric,minConfidence=0){
  return metric?.status==='MEASURED'&&Number.isFinite(Number(metric.score))&&Number(metric.confidence||0)>=minConfidence;
}

function cyclePotentialScore({events=[],actorRows=[],scoutRows=[],independent=[],scouts=[],exits=[],distribution=false,safetyResult={},exitabilityResult={},exitability=0,currentMc=null,firstMc=null,cycleMetrics={}}={}){
  ...
  const holdersMeasured=measuredMetric(holderMetric,0.25);
  const resilienceMeasured=measuredMetric(resilienceMetric,0.25);
  const holders=holdersMeasured?Math.round(clamp(Number(holderMetric.score))):null;
  const resilience=resilienceMeasured?Math.round(clamp(Number(resilienceMetric.score))):null;

  const rawV1=0.25*money+0.22*culture+0.15*velocity.score+0.14*market+0.10*safety+0.08*survival+0.06*momentum;
  let adjusted=rawV1;
  if(holdersMeasured)adjusted=0.88*adjusted+0.12*holders;
  if(resilienceMeasured)adjusted=0.88*adjusted+0.12*resilience;

  const holdersReady=measuredMetric(holderMetric,0.55);
  const resilienceReady=measuredMetric(resilienceMetric,0.50);
  const fullDataReady=holdersReady&&resilienceReady;
  let score=Math.round(clamp(adjusted));
  if(!fullDataReady)score=Math.min(84,score);

  let stage=cycleStage(score);
  const chainIconGate=fullDataReady&&safetyResult.status==='PASS'&&culture>=70&&holders>=55&&resilience>=55;
  if(['CHAIN_ICON','CYCLE_MEME'].includes(stage)&&!chainIconGate)stage='CULTURAL_CONTENDER';
  const cycleMemeGate=chainIconGate&&Number(holderMetric.confidence||0)>=0.75&&Number(resilienceMetric.confidence||0)>=0.70&&culture>=82&&(money>=65||holders>=85);
  ...
  if(holdersMeasured&&holders>=70)reasons.push('holder growth/distribution improving');
  if(resilienceMeasured&&resilience>=70)reasons.push('post-drawdown resilience measured');
  ...
  if(holdersMeasured&&holders<45)risks.push('holder growth/distribution weakening');
  if(resilienceMetric?.status==='MEASURED'&&resilience<45)risks.push('post-drawdown structure remains weak');
  if(resilienceMetric?.status==='TESTING')risks.push('post-drawdown resilience still being tested');

  const missingData=[];
  if(!holdersReady)missingData.push('holder_growth');
  if(!resilienceReady)missingData.push('long_horizon_resilience');
  return{
    score,stage,velocity:velocity.score,direction:velocity.direction,
    components:{money,culture,velocity:velocity.score,market,safety,survival,momentum,holders,resilience},
    reasons:reasons.slice(0,5),risks:risks.slice(0,5),missingData,
    evidence:{
      moneyActors:independent.length,socialScouts:scouts.length,recentEvents6h:velocity.recentEvents,priorEvents18h:velocity.priorEvents,
      holdersStatus:holderMetric?.status||'UNKNOWN',holdersConfidence:Number(holderMetric?.confidence||0),currentHolders:holderMetric?.currentHolders??null,top10Pct:holderMetric?.top10Pct??null,
...
function evaluateToken({events=[], actors={}, safety={}, execution={}, token={}, cycleMetrics={}}){
  ...
  const cycle=cyclePotentialScore({events,actorRows,scoutRows,independent,scouts,exits,distribution,safetyResult,exitabilityResult,exitability,currentMc,firstMc,cycleMetrics});
...
module.exports = {sampleConfidence,activityFactor,division,adaptiveActorScore,independentActors,safetyGate,exitabilityScore,exitabilityGate,timingScore,cycleStage,eventVelocity,measuredMetric,cyclePotentialScore,evaluateToken,clamp,hours};
