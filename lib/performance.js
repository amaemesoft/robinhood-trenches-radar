...
function socialScoutCalibration(actor,actors=[],events=[],now=Date.now(),windowMinutes=SCOUT_CONFIRMATION_WINDOW_MINUTES){
  ...
  return{
    ...
    confirmationRate:resolved?round(confirmed.length/resolved*100):null,
    medianLeadMinutes:median(confirmed.map(row=>row.leadMinutes)),
...
function calibrationReport(actors=[],events=[],marketHistory={}){
  ...
  return{
    ...
    policy:{
      ...
      socialPromotion:'social scout scores require resolved discovery sample; SCOUT never counts as economic entry',
...
module.exports={ENTRY_ACTIONS,EXIT_ACTIONS,SCOUT_CONFIRMATION_WINDOW_MINUTES,HORIZONS,median,returnPct,eventPerformance,actorCalibration,socialScoutCalibration,calibrationReport};
