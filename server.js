...
const RH_WSS_URL=process.env.RH_WSS_URL||'wss://rpc.mainnet.chain.robinhood.com';
...
function initial(){const now=new Date().toISOString();return{version:3,createdAt:now,updatedAt:now,actors:seedActors,events:[],tokenState:{},alerts:[],sync:{lastChainSync:null,lastScannedBlock:null,lastError:null,lastScan:null,ws:null,lastLiveTx:null}}}
...
async function enrichAndStore(rows,txHash){
  ...
  for(const e of rows){
    ...
    const market=await scanner.market(e.tokenAddress);
    if(market){
      mergeTokenState(e.tokenAddress,{marketCap:market.marketCap,priceUsd:market.priceUsd,fdv:market.fdv,execution:{liquidityUsd:market.liquidityUsd,pairAddress:market.pairAddress,dexId:market.dexId,marketSource:market.marketSource}});
      e.marketCap=market.marketCap;e.liquidityUsd=market.liquidityUsd;
    }
...
async function ingestLiveTx(txHash){
  ...
  return enrichAndStore(rows,txHash);
}

function queueLiveTx(txHash){
  liveQueue=liveQueue.then(()=>ingestLiveTx(txHash)).catch(e=>{
    db.sync={...(db.sync||{}),lastError:e.message,lastLiveTx:txHash};save();
    console.error(`[live] ${txHash} ${e.message}`);
  });
  return liveQueue;
}
...
async function runLiveSync(){
  ...
  syncRunning=true;
  try{
    const result=await scanner.scan({actors:db.actors,sync:db.sync||{}});
    db.sync={...(db.sync||{}),lastChainSync:new Date().toISOString(),lastScannedBlock:result.toBlock??db.sync?.lastScannedBlock??null,lastError:null,chainId:result.chainId||4663,lastScan:{fromBlock:result.fromBlock,toBlock:result.toBlock,transactions:0,newEvents:0,trackedWallets:result.trackedWallets||0,discovery:result.discovery||'wss-live'}};
    save();await storage.flush();
...
function startLiveSubscriber(){
  liveSubscriber=new LiveSubscriber({
    wsUrl:RH_WSS_URL,
    getWallets:()=>[...moneyWalletMap().keys()],
    onTx:queueLiveTx,
    onStatus:s=>{db.sync={...(db.sync||{}),ws:s};save();console.log(`[wss] ${s.state}${s.error?' '+s.error:''}`);}
  });
...
if(p==='/api/health')return json(res,200,{ok:true,version:4,storage:DATABASE_URL?'postgres':'file',chainId:4663,scanner:'robinhood-wss+rpc',ws:db?.sync?.ws?.state||'starting',time:new Date().toISOString()});
