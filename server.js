...
const ShadowRuntime=require('./lib/shadowRuntime');
...
async function storeEvents(rows,tokenStates={}){
  ...
  for(const e of rows||[]){
    ...
    e.receivedAt=e.receivedAt||new Date().toISOString();fresh.push(e);
  }
...
async function buildTokenState(token,force=false){
  ...
  }:{priceUsd:null,marketObservedAt:null};
...
if(p==='/api/events'&&req.method==='POST'){if(!auth(req,WRITE_API_TOKEN))return json(res,401,{error:'unauthorized'});const b=await body(req);if(!b.actorId||!b.action||!/^0x[a-fA-F0-9]{40}$/.test(b.tokenAddress||''))return json(res,400,{error:'actorId, action and exact tokenAddress required'});if(!db.actors.some(a=>a.id===b.actorId))return json(res,404,{error:'actor not found'});if(b.key&&db.events.some(e=>e.key===b.key))return json(res,200,{ok:true,deduplicated:true});const e={receivedAt:new Date().toISOString(),key:b.key||`${Date.now()}:${b.actorId}:${b.tokenAddress}`,actorId:b.actorId,action:String(b.action).toUpperCase(),tokenAddress:b.tokenAddress.toLowerCase(),symbol:b.symbol||null,marketCap:b.marketCap??null,blockNumber:b.blockNumber??null,blockHash:b.blockHash??null,at:b.at||new Date().toISOString(),signalRole:b.signalRole||null,source:b.source||'external'};db.events.unshift(e);save();queueCycleRefresh();return json(res,201,{ok:true})}
