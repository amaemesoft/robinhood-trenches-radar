...
function initial(){const now=new Date().toISOString();return{version:3,createdAt:now,updatedAt:now,actors:seedActors,events:[],tokenState:{},alerts:[],sync:{lastChainSync:null,lastError:null}}}
...
if(p==='/api/health')return json(res,200,{ok:true,version:3,storage:DATABASE_URL?'postgres':'file',time:new Date().toISOString()});
if(p==='/api/dashboard'&&req.method==='GET')return json(res,200,dashboard());
if(p==='/api/internal/sync'&&req.method==='POST'){if(!auth(req,INTERNAL_SYNC_TOKEN))return json(res,401,{error:'unauthorized'});db.sync.lastChainSync=new Date().toISOString();db.sync.lastError=null;save();return json(res,200,{ok:true,note:'worker heartbeat accepted; live source adapters are the next layer'})}
if(p==='/api/events'&&req.method==='POST'){if(!auth(req,WRITE_API_TOKEN))return json(res,401,{error:'unauthorized'});const b=await body(req);if(!b.actorId||!b.action||!/^0x[a-fA-F0-9]{40}$/.test(b.tokenAddress||''))return json(res,400,{error:'actorId, action and exact tokenAddress required'});if(!db.actors.some(a=>a.id===b.actorId))return json(res,404,{error:'actor not found'});const e={key:b.key||`${Date.now()}:${b.actorId}:${b.tokenAddress}`,actorId:b.actorId,action:String(b.action).toUpperCase(),tokenAddress:b.tokenAddress.toLowerCase(),symbol:b.symbol||null,marketCap:b.marketCap??null,at:b.at||new Date().toISOString(),signalRole:b.signalRole||null,source:b.source||'external'};db.events.unshift(e);save();return json(res,201,{ok:true})}
if(p==='/api/token-state'&&req.method==='POST'){if(!auth(req,WRITE_API_TOKEN))return json(res,401,{error:'unauthorized'});const b=await body(req);if(!/^0x[a-fA-F0-9]{40}$/.test(b.tokenAddress||''))return json(res,400,{error:'exact tokenAddress required'});db.tokenState[b.tokenAddress.toLowerCase()]={...(db.tokenState[b.tokenAddress.toLowerCase()]||{}),...(b.state||{})};save();return json(res,200,{ok:true})}
...
}catch(e){db.sync.lastError=e.message;save();return json(res,500,{error:e.message})}});

(async()=>{db=await storage.init(initial(),x=>x?.version===3?x:{...initial(),...x,version:3});save();server.listen(PORT,()=>console.log(`Trenches Radar listening on ${PORT} (${DATABASE_URL?'postgres':'file'})`))})().catch(e=>{console.error(e);process.exit(1)});
